...
# 4x4 chunk grid oriented so the camera-facing "forward" (-Z / north) leads INTO the world:
# you spawn in the city at the SOUTH (gz=3) and walk forward (north) through a park + forest
# down to the beach at gz=0, with the ocean beyond the north edge.
import json, random
random.seed(7)

CITY_GROUND   = [0.40, 0.40, 0.43]
PARK_GROUND   = [0.34, 0.39, 0.34]
FOREST_GROUND = [0.26, 0.33, 0.18]
...
def road(): return {"prim":"box","pos":[0,0],"size":[7.2,0.06,20.0],"color":[0.20,0.20,0.22],"y":0.01}
...
def wetsand(z): return {"prim":"box","pos":[0,z],"size":[20.0,0.05,5.0],"color":[0.66,0.62,0.50],"y":0.0}
...
# ---------------- ROW gz=3 : CITY CORE (spawn; player looks north into the world) ----------------
addcell(0,3,"city",CITY_GROUND,
...
addcell(2,3,"city",CITY_GROUND,
    ...
    buildings=[{"type":"lobby","pos":[-5.8,-4.2],"rot":0,"place":"lobby","name":"Apartment Lobby","h":22,"seed":4.0},
               tower(6.5,-6.8,6,6,19,4),cab(0.0,8.0,180)],
    pedestrians=[{"pos":[-6,3]},{"pos":[7,4]},{"pos":[2,6]}])
addcell(3,3,"city",CITY_GROUND,
    ...
    buildings=[{"type":"diner","pos":[0.0,-4.5],"rot":0,"place":"diner","name":"The Diner"},
               tower(-7.0,6.8,6,6,20,5),tower(7.0,6.8,5.5,5.5,16,1),cab(7.5,2.0,172)],
    pedestrians=[{"pos":[-6,3]},{"pos":[6,6]}])

# ---------------- ROW gz=2 : CITY EDGE / PARK TRANSITION ----------------
addcell(0,2,"city",PARK_GROUND,
...
addcell(2,2,"city",PARK_GROUND,
    props=[road(),grass(6,4,8),grass(-6,5,7),bench(6,4),light(-7,7),rockp(7,6),cab(-7.5,7.0,185)],
    scatter=[scat("tree",6),scat("bush",4)],
    buildings=[tower(7.0,7.2,5.5,5.5,17,2,crown=False)],
    pedestrians=[{"pos":[-5,3]},{"pos":[6,6]}])
addcell(3,2,"city",PARK_GROUND,
...
for gx in range(4):
    ...
    addcell(gx,1,"forest",FOREST_GROUND,
        props=pr,
        scatter=[scat("pine",10),scat("tree",8),scat("birch",6),scat("bush",8),scat("rock",4)],
        pedestrians=peds)

# ---------------- ROW gz=0 : BEACH (shoreline + ocean at the NORTH edge, local -z) ----------------
for gx in range(4):
    ...
    addcell(gx,0,"beach",BEACH_GROUND,
        props=pr,
...
world={
    ...
    "grid":{"cell_size":20},
    "start_cell":[1,3],
    "goal":{"type":"reach_cell","target":[2,0]},
    "ambient":[0.62,0.66,0.72],
    "cells":cells,
}
with open("world.json","w") as f:
    json.dump(world,f,indent=1)
print("wrote world.json with", len(cells), "cells; spawn city gz=3 -> beach gz=0 (north)")
